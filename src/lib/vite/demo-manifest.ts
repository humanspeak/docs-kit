...
type DemoDependencyGraph = Record<string, string[]>
 ...
 * that lives under a `demos/` segment somewhere on its path. These files
 * are the public demo entrypoints; co-located component files are pulled in
 * later by following each demo's local import graph.
 */
...
function toManifestKey(examplesRoot: string, file: string): string {
    return relative(examplesRoot, file).split(sep).join('/')
}

function getExampleRoot(examplesRoot: string, demoFile: string): string {
    const [exampleSlug] = toManifestKey(examplesRoot, demoFile).split('/')
    return resolvePath(examplesRoot, exampleSlug)
}
...
async function collectLocalDemoSources(
    demoFiles: string[],
    examplesRoot: string
): Promise<{ files: string[]; dependencies: DemoDependencyGraph }> {
    const files = new Set(demoFiles)
    const dependencies: DemoDependencyGraph = {}
    ...
    const visit = async (
        ...
        for (const specifier of localSvelteImports(source)) {
            ...
            if (!isInsideDir(resolved, exampleRoot)) continue
            if (!resolved.endsWith('.svelte')) continue
            if (!existsSync(resolved)) continue

            files.add(resolved)
            deps.push(resolved)
            deps.push(...(await visit(resolved, exampleRoot, seen)))
        }
    ...
    for (const demoFile of demoFiles) {
        const exampleRoot = getExampleRoot(examplesRoot, demoFile)
        const demoKey = toManifestKey(examplesRoot, demoFile)
        dependencies[demoKey] = (await visit(demoFile, exampleRoot, new Set()))
            .filter((dep) => dep !== demoFile)
            .map((dep) => toManifestKey(examplesRoot, dep))
    }
...
async function buildManifestJson(
    ...
    for (const file of files) {
        const rel = toManifestKey(examplesRoot, file)
...
function buildDemoLoadersTs(
    manifest: DemoManifest,
    dependencies: DemoDependencyGraph,
    options: ResolvedOptions
): string {
    const lines = [
    ...
    for (const key of Object.keys(dependencies).sort()) {
        const deps = dependencies[key].filter((dep) => dep in manifest)
        if (deps.length === 0) continue
        lines.push(`    '${key}': [${deps.map((dep) => `'${dep}'`).join(', ')}],`)
    }
...
export function demoManifestPlugin(userOptions: DemoManifestOptions = {}): Plugin {
    ...
    async function regenerate() {
        const demoFiles = await findDemoFiles(examplesAbs)
        const { files, dependencies } = await collectLocalDemoSources(demoFiles, examplesAbs)
        ...
        if (opts.split && typeof loaderOutputAbs === 'string') {
            const loaderTs = buildDemoLoadersTs(manifest, dependencies, opts)
            let currentLoader = ''
     ...
     *  it must live under the examples dir and be a Svelte source file. */
    function isWatchedExampleSvelteFile(absPath: string): boolean {
        if (!absPath.startsWith(examplesAbs + sep)) return false
        return absPath.endsWith('.svelte')
    }
    ...
    return {
        ...
        configureServer(server: ViteDevServer) {
            // Make sure Vite's watcher knows about every demo file and any
            // imported local component even if no module currently imports
            // them — otherwise a brand-new file's first save wouldn't fire
            // `change`.
            findDemoFiles(examplesAbs).then(async (demoFiles) => {
                const { files } = await collectLocalDemoSources(demoFiles, examplesAbs)
                for (const file of files) server.watcher.add(file)
                // Also watch the examples dir itself so newly created
                // demo/component files get picked up.
                server.watcher.add(examplesAbs)
            })

            const onWatcherEvent = async (file: string) => {
                if (!isWatchedExampleSvelteFile(file)) return
