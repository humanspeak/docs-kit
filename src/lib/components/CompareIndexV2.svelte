<!--
  ...
  Mounts on `/compare/+page.svelte`. Renders the coordinate strip, hero,
  3-column comparison grid (one cell per competitor), and the big-type
  footer. Pairs with `ComparisonPageV2` for the per-competitor pages.
  ...
  Must mount inside a brutalist wrapper (the per-site `/compare/+layout.svelte`
  using `CompareLayoutV2` already provides this).
-->
<script lang="ts">
    ...
    const footerLabelDefault = $derived.by(() => {
    ...
    const footerLabel = $derived(footerCta?.label ?? footerLabelDefault)
...
<main class="brut">
    ...
    <section class="brut-hero">
        ...
        <aside class="meta">
            ...
            <hr />
            <div>
                <span class="k">library</span> · <span class="v">{ours.npmPackage}</span>
            </div>
            <div>
                <span class="k">framework</span> · <span class="v accent">{framework}</span>
            </div>
            <hr />
            <div class="k">// scroll for matrices</div>
        </aside>
        <div class="hero-body">
            <div class="k">// compare / {ours.slug} vs the field</div>
            <h1>
                <span>compare</span><span class="end">.</span>
            </h1>
            <p class="sub">
                <!-- eslint-disable-next-line svelte/no-at-html-tags -- consumer-supplied or built from competitor names -->
                {@html introHtml ?? defaultIntro}
            </p>
            <div class="cta-row">
